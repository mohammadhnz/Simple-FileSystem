...
int searchPathInInode(int inodeNumber, char *search, int *outputInodeNumber) {
    ...
    for (i = 0; i < SECTOR_PER_FILE_MAX; i++) {
        //find sector number
        memcpy((void *) inodeSegmentPointerToSector,
               (void *) inodeBuffer + META_DATA_PER_INODE_BYTE_NUM + i * sizeof(int), sizeof(int));
...
int DeleteEntryFromDirectory(int inodeNumber, int inodeSearch) {
    ...
    for (i = 0; i < SECTOR_PER_FILE_MAX; i++) {
        ...
        if (i < SECTOR_PER_FILE_MAX - 1) {
            memcpy((void *) inodeSegmentPointerToNextSector,
                   (void *) inodeBuffer + META_DATA_PER_INODE_BYTE_NUM + (i + 1) * sizeof(int), sizeof(int));
        ...
        for (j = 0; j < (SECTOR_SIZE) / DIRECTORY_LENGTH; j++) {
            ...
            if (isFind == 1) {
                if (j < ((SECTOR_SIZE / DIRECTORY_LENGTH) - 1)) {
                    memcpy(sectorBuffer + j * DIRECTORY_LENGTH, sectorBuffer + (j + 1) * DIRECTORY_LENGTH,
                           DIRECTORY_LENGTH);
